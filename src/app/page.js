...
const defaultCategories = ["Frontend", "Backend", "Testing"];
const defaultGroupedSkills = {
  "Frontend": [
      ...
      {row.map((skill) => (
        ...
        >
          <span style={{ display: 'flex', alignItems: 'center', marginRight: 'var(--skill-padding-y)' }}>
            {skill.iconUrl ? (
              <img src={skill.iconUrl} alt={skill.name} style={{ width: '18px', height: '18px', objectFit: 'contain' }} />
            ) : (
...
const ProjectImage = ({ src, title, alt }) => {
  const [loaded, setLoaded] = React.useState(false);
  return (
    <div className={`project-card-img-box ${loaded ? '' : 'img-box-skeleton'}`} style={loaded ? { background: 'none' } : {}}>
      <img
        src={src}
        title={title}
        alt={alt}
        className={`project-card-img ${loaded ? 'img-loaded' : 'show-img-skeleton'}`}
...
// Experiences data fetched from API
const projectsData = [
  {
    name: 'FyleHQ - Clone',
...
export default function Home() {
  ...
  // About section state — fetched from TiDB via admin panel
  const [aboutData, setAboutData] = React.useState({
    heading: 'Behind the Developer',
    description: 'I am Rishi, a Full-Stack Developer specializing in architecting scalable web applications using the MERN stack (MongoDB, Express.js, React, Node.js). I engineer responsive frontends, RESTful APIs, and robust backend systems. I am also proficient in Python for backend development and automation workflows.',
    resume_btn_text: 'Open my CV',
    resume_link: '/assets/Rishi_Resume.pdf',
    profile_image_url: './assets/images/about/Rishi Proffess pic.png',
  });
  ...
  const [isLoadingExps, setIsLoadingExps] = React.useState(true);

  const [projectsList, setProjectsList] = React.useState([]);
  const [isLoadingProjs, setIsLoadingProjs] = React.useState(true);

  const [socialData, setSocialData] = React.useState({
    linkedin: 'https://www.linkedin.com/in/rishiaravindha',
    github: 'https://github.com/Rishidevlx',
    mail: 'rishiaravindhaoff@gmail.com',
    whatsapp: 'https://wa.link/5apdqm',
    mobile: '+91 8590605554'
  });

  const [contactData, setContactData] = React.useState({
    email: 'rishiaravindhaoff@gmail.com',
    phone: '+91 8590605554',
    location: '138, mudhaliyaar patti street,\nsrivilliputhur, virudhunagar,\nTamilnadu, India'
  });
  ...
  React.useEffect(() => {
    ...
    // Fetch Social Links
    fetch('/api/sociallinks', { cache: 'no-store' })
      .then((r) => r.json())
      .then((json) => {
        if (json.success && json.data) {
          setSocialData({
            linkedin: json.data.linkedin || socialData.linkedin,
            github: json.data.github || socialData.github,
            mail: json.data.mail || socialData.mail,
            whatsapp: json.data.whatsapp || socialData.whatsapp,
            mobile: json.data.mobile || socialData.mobile
          });
        }
      })
      .catch(() => { });
    ...
    // Fetch Contact Info
    fetch('/api/contactinfo', { cache: 'no-store' })
      .then((r) => r.json())
      .then((json) => {
        if (json.success && json.data) {
          setContactData({
            email: json.data.email || contactData.email,
            phone: json.data.phone || contactData.phone,
            location: json.data.location || contactData.location
          });
        }
      })
      .catch(() => { });
    ...
    fetch('/api/experiences', { cache: 'no-store' })
      ...
      .then((json) => {
        if (json.success && json.data) {
          setExperiencesList(json.data);
        }
      })
      .catch(() => { })
      .finally(() => setIsLoadingExps(false));
    ...
    fetch('/api/projects', { cache: 'no-store' })
      ...
      .catch(() => { })
      .finally(() => setIsLoadingProjs(false));

    // Fetch Skills Data
    fetch('/api/skills', { cache: 'no-store' })
      .then((r) => r.json())
      .then((json) => {
        if (json.success && json.data && json.data.categories.length > 0) {
          setSkillCats(json.data.categories);
          setGroupedSkills(json.data.groupedSkills);
        } else {
          setSkillCats(defaultCategories);
          setGroupedSkills(defaultGroupedSkills);
        }
      })
      .catch(() => {
        setSkillCats(defaultCategories);
  ...
  const totalProjPages = Math.ceil(projectsList.length / itemsPerProjPage) || 1;

  const activeProjects = projectsList.slice(
  ...
  const totalExPages = Math.ceil(experiencesList.length / itemsPerExPage);

  const activeExperiences = experiencesList.slice(
  ...
  }, [currentExPage, currentProjPage, experiencesList, projectsList]);
        ...
        <div className="bg-black-blur" style={{ padding: '1rem 1rem' }}>
          ...
          <div id="About-me" className="about-clean-container">
            <div className="about-clean-wrapper">
              <h2 className="about-clean-heading">{aboutData.heading}</h2>
            ...
            <div className="skills-split-layout">
              ...
              <div className="skills-left-column">
                <section id="skill-section">
                  {isLoadingSkills ? (
                    <div style={{ padding: '2rem', textAlign: 'center', opacity: 0.6, fontSize: '0.9rem', color: 'var(--body-text-color)' }}>
                    ...
                    skillCats.map(cat => {
                      ...
                      return (
                        <div className="skill-container" key={cat}>
                          <h3>{cat}</h3>
                          <div className="skills">
                            {renderSkillGroup(catSkills)}
                          </div>
                        </div>
                      );
                    })
                          ...
                          {exp.skills.map((skill, sIdx) => {
                            return (
                              <a
                                ...
                                rel="noopener noreferrer"
                              >
                                <span style={{ display: 'flex', alignItems: 'center', aspectRatio: '1' }}>
                                  {skill.iconUrl ? (
                                    <img src={skill.iconUrl} alt="skill" style={{ width: '18px', height: '18px', objectFit: 'contain' }} />
                                  ) : (
                                    <span className={skill.iconClass}></span>
                                  )}
                                </span>
                              </a>
                            );
        ...
        <div className="min-width-400px-width-50per" id="Projects" style={{ background: 'var(--body-color)' }}>
          ...
          <section id="project-section" className="project-section " style={{ padding: '1rem' }}>
            {activeProjects.map((proj) => {
              return (
                <div className="project-card in-view" key={proj.name}>
                  <ProjectImage src={proj.image} title={proj.title} alt={proj.title} />
                  ...
                  <div className="project-card-footer">
                    <div className="project-skills">
                      {proj.skills.map((skill, sIdx) => (
                        <div className="skill-icon" key={sIdx}>
                          <span
                            style={{ display: 'flex', alignItems: 'center', aspectRatio: '1' }}
                          >
                            {skill.icon ? (
                              <img src={skill.icon} alt="tech icon" style={{ width: '18px', height: '18px', objectFit: 'contain' }} />
                            ) : (
                              <span className={skill.iconClass}></span>
                            )}
                          </span>
